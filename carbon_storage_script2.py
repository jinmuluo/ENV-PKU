...
Caddress2006 = 'E:/ModisFire/Carbonstorage/carbon2006.tif'
Carbon_2006 = gl.Open(Caddress2006)
if Carbon_2006 == None:
    print('Tiff file is not exits')
im_width = Carbon_2006.RasterXSize
im_height = Carbon_2006.RasterYSize
im_bands = Carbon_2006.RasterCount
carbon_term = Carbon_2006.ReadAsArray(0, 0, im_width, im_height)
np.nan_to_num(carbon_term)
carbon_S2006 = np.zeros([720, 1440])
carbon_S2006[0:im_width, 0:im_height] = carbon_term
"""""
In this section,  we load the npp data form carbon_storage.py;
Notice, The NPP data in unit: tons every grid and the resolution is 0.25 degree x 0.25 degree, yearly.
"""
NppAddress = 'G:/PAHs/'
ModisNameList = find.find_file(NppAddress)
...
"""
Construct the Grid area in unit km2 matrix here.
...
for i in range(2001, 2015):
    ...
    for j in range(len(ModisNameList)):
        term_name = ModisNameList[j][name_long - 23:name_long]
        if int(term_name[16:19]) == i:
            new_name_list.append(ModisNameList[j])
    print('we have', len(new_name_list), 'files in year:', i)
    npp = Dataset(new_name_list, 'r', format='NetCDF4')
    npp_global_add = npp.variables['npp'][:]
    print(type(npp_global_add))
    print(npp_global_add.shape)
